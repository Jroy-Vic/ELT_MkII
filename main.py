...
SERVOX_CAL = (50, 500, 650, 875)            # Calibration for ServoX
SERVOY_CAL = (50, 500, 775, 550)            # Calibration for ServoY
...
ARRAY_SIZE = 6                              # Size of DMA Stream
...
def write_MCU():
    ...
    while not Exit_Flag.is_set():
        ...
        if Servo_iter == SERVOX_ITER:
            placeHolder = (ARRAY_SIZE - len(dataArr[0]) - 1)
            user_input = f"X{dataArr[0]}"
            user_input += '\n' * placeHolder
            Servo_iter = SERVOY_ITER
        else:
            placeHolder = (ARRAY_SIZE - len(dataArr[1]) - 1)
